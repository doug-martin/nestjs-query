...
In `v0.11.x` two new decorators were introduced from the `@codeshine/nestjs-query-query-graphql` package. The `@Relation` and `@Connection` allow specifying relations on the DTO rather than passing them in as options to the `CRUDResolver`.
...
In `v0.11.x` the `@codeshine/nestjs-query-query-graph` package is now able to automatically create your resolvers.
