...
:::info
This section only applies if you are manually defining your Resolver.
If you are using the `NestjsQueryGraphQLModule` an `AssemblerQueryService` will automatically be created.
:::
...
This example wraps a `QueryService` to assemble to `UserDTO` and `UserEntity`.
