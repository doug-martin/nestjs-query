...
:::note
In the above example the DTO and entity are the same shape, if you have a case where they are different or have computed fields check out [Assemblers](../concepts/assemblers.mdx) to understand how to convert to and from the DTO/Entity.
:::
...
Instead decorate the class with `@Connection` or `@Relation`.
