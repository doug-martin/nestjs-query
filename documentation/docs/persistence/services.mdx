...
The `nestjs-query` `typeorm` and `sequelize` packages provide a module that will add providers to inject auto-created `QueryServices` using the `@InjectQueryService` decorator.
