...
**NOTE** By default `CRUDResolver` will use the name used by `type-graphql` when the DTO is annotated with `@ObjectType`.
...
Notice how `TodoItemResolver` extends `CRUDResolver` and the class is annotated with `@Resolver`. Without the annotation
 the resolver **WILL NOT** be exposed in the graphql schema.
