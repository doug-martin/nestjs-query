...
All examples will be based on the following `TodoItem` DTO. All fields except `age`, because it is not decorated with `@FilterableField`, will be exposed in aggregate queries.
