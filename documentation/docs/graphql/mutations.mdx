...
The [CRUDResolver](./resolvers) automatically exposes six mutation endpoints. The endpoints names will be derived
...
The `CRUDResolver` will by default expose an `updateOne` mutation that takes two arguments:

* `id`: The id of the record to update.
* `input`: The values to update on the record. This is a partial so you only have to pass in the values you want to
change.
...
The `CRUDResolver` will by default expose an `updateMany` mutation that takes two arguments:
