...
:::note
 In this example the DTO and entity are the same shape, if you have a case where they are different or have computed fields check out [Assemblers](../concepts/assemblers.mdx) to understand how to convert to and from the DTO/Entity.
:::
...
* `pagingStrategy?` - Specify to use an alternate paging strategy (`CURSOR`, `OFFSET`). See [Paging Strategy](#paging-strategy).
...
You can override the default `pagingStrategy` to an alternate strategy such as `OFFSET`.

When using the `OFFSET` strategy your the paging arguments for a many query will accept a `limit` and/or `offset`. This will also change the return type from a `Connection` to a list of items.
...
export class TodoItemResolver extends CRUDResolver(TodoItemDTO) {
  ...
  completedTodoItems(@Args() query: TodoItemQuery): Promise<ConnectionType<TodoItemDTO>> {
    ...
    // call the original queryManyConnection method with the new query
    return this.queryManyConnection({ ...query, ...{ filter } });
  }
}
...
Finally we call the `queryManyConnection` method from the `CRUDResolver` with the new filter set on the query.
