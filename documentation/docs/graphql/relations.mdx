...
The `RelationQueryService` allows you to create `virtual` relations on a DTO by wrapping the original `QueryService`
and specifying relations. When a relation look up is called it will:
* First check if the relation is a virtual relation, if true it will call the `query` option and pass that to the
provided relation `queryService` to look up the relations.
...
Once the `relation` is defined in the query service we can add it to our `DTO`
